...
# load images
bg = pygame.image.load('main2.png').convert_alpha()
# castle
castle_img_100 = pygame.image.load('castle100.png').convert_alpha()
castle_img_50 = pygame.image.load('castle_50.png').convert_alpha()
castle_img_25 = pygame.image.load('castle_25.png').convert_alpha()

# tower
tower_img_100 = pygame.image.load('tower_100.png').convert_alpha()
tower_img_50 = pygame.image.load('tower_50.png').convert_alpha()
tower_img_25 = pygame.image.load('tower_25.png').convert_alpha()
...
pygame.mixer.music.load('Call to Adventure.mp3')  # Replace with your music file path
    ...
    if not game_over:
        ...
        if next_level:
            ...
            if pygame.time.get_ticks() - level_reset_time > 1500:
                ...
                level += 1
                last_enemy = pygame.time.get_ticks()
                if level > 4:  # Check if level exceeds 4
                    draw_text('GAME COMPLETE!', font_60, WHITE, 250, 350)
        ...
        if key[pygame.K_a]:
            ...
            castle.money = 0
            pygame.mouse.set_visible(False)
            start_screen()  # Show start screen after game over
            instructions_screen()  # Show instructions screen again

    # event handler
    for event in pygame.event.get():
        if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_q):
            run = False
        elif event.type == pygame.KEYDOWN and event.key == pygame.K_a and game_over:
            # reset variables
            game_over = False
            level = 1
            target_difficulty = 1000
